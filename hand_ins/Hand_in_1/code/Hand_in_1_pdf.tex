...
We find that the script returns a value for every test input without any hardcoding, and we believe this achieves a high accuracy at returning 7 significant figures. The results of this can be seen in table. \ref{tab:poisson}. I originally struggled with the high factorial inverse under flowing, but the log factorial and rewritten poisson distribution handles the data exceptionally well.
...
The results for this section have all been stacked on the same graph, but the results are evident and the use of markers make it possible to distingish the difference between overlapping polynomials. The results for this can be seen in fig. \ref{fig:q2results}. We can see here that the 19 degree polynomials all closely mimic each other with Neville's having the lowest error, on the scale of $10^{-13}$ where as the error on the iterations of the LU decomp are quite high, the same as the year before, so there may be an issue here with either the get\_y function or the LU decomposition itself which will require more study in order to have this function working correctly for the following assignments. We also don't see any improvement on the 10th iteration of the LU decomp and the reason for this is unknown, given the error is quite high we should expect it to improve, but from my understanding, Crout's algortithm finds the solution decomposition of a matrix to any vector on the first attempt so I think that it is the case that iterating on Crout's should yield the same result. I did discover during my experimentation that if I re-estimated Y and input this into the forward-backward substitution, the error would grow exponentially, leading me to believe there is a key flaw somewhere in my algorithm, and it may be purely coincidental that my implementation works, this needs further study.

The times of each implementation can be see in table. \ref{tab:time}. As we can see crouts implementation is much faster than Neville's algorithm, and this is to be expected as we are solving a 20x20 set of equations, instead of looping through a thousand points and performing an interpolation on each point. So I believe it is imperative that we refine the crouts method for future work.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{../plots/results.png}
    \caption{Caption}
    \label{fig:q2results}
\end{figure}
...
\begin{table}
    ...
    \hline
        Method & Time (sec) \\
        \hline
        1st iteration Crout's & \input{../data/1_iter_time.txt}\\
        10th iteration Crout's & \input{../data/10_iter_time.txt}\\
        Neville's & \input{../data/nev_time.txt}\\
        \hline
    \end{tabular}
    \caption{Timing of each implementation}
    \label{tab:time}
\end{table}
