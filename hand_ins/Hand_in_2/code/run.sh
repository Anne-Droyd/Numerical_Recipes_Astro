...
echo "Running Python script to solve the satellite galaxy number density in 1a..."
python3 Q1.py
...
cat Q1.py > Code/Poisson_code.txt
cat Q1.py > Code/satellites_integrator_code.txt
cat Q1.py > Code/satellites_sampling_code.txt
cat Q1.py > Code/satellites_selection_code.txt
cat Q1.py > Code/satellites_derivative_code.txt

echo "Running the Python script to solve the heating and cooling question 2..."
python3 Q2.py

# Copy the code to a text file which will be shown in the PDF
cat Q2.py > Code/heatingcooling_code.txt

echo "Compiling LaTeX..."
pdflatex -interaction=batchmode NURA_handin_2.tex
pdflatex -interaction=batchmode NURA_handin_2.tex &>/dev/null # Run a second time to fix links/references

pdfs=($(ls -1 *.pdf 2> /dev/null))
if [[ "${#pdfs[@]}" -eq 0 ]] ; then
    echo "No PDF was produced; check the .log file for errors."
else
    echo "run.sh completed! Don't forget to hand in a *clean* version of this directory (remove downloaded data, the Plots, Code and Calculations directories)."
fi
